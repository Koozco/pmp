...
#
#
# def clustering(election, k, committee=None, scoring=None, iterations=30, restart=True, fill_greedy=False,
#                init_alpha=False, alpha=1):
#     '''
#         currently ignoring the order if scoring doesn't break ties,
#         see scoring with all ones
#     '''
...
#
#     return (best_committee, best_score, np.mean(restarts)) if best_committee is not None else (
#     committee, score_committee(election, committee, scoring), iterations_since_convergence)
#
#
...
#         iterations_since_convergence += 1
#
#     return (best_committee, best_score, 0) if best_committee is not None else (
#     committee, score_, iterations_since_convergence)
#
#
...
#             delayed(run_greedy_monroe_noc)(election, k, d, scoring=scoring) for _ in range(5))
#
#     for (s, cs) in res:
#         scores.append(cs)
#         if s > best_score_nc:
...
#         committee, score, _ = clustering_monroe(election, k, committee=c, committee_score=scores_[i], scoring=scoring,
#                                                 restart=False)
#         committees_.append(committee)
#         scores.append(score)
#         if score > best_score:
...
#                         lowest = add_to_pq_hack(tmp_partial,
#                                                 (candidate_score, (pc_, rest, candidate, idx_left, votes_idx)), d, size)
#
#                     if size < d:
#                         size += 1
#
...
#
#                 if size < d or (size == d and candidate_score > lowest):
#                     lowest = add_to_pq_hack(tmp_partial, (candidate_score, (pc_, rest, candidate, idx_left, votes_idx)),
#                                             d, size)
#
#                 if size < d:
#                     size += 1
...
#
#
# def write_cplex_format_OWA_egal(election, k, OWA, out, scoring=None):
#     n, m = election.shape
#
#     if scoring is not None:
#         assert m == len(scoring), "mismatch: number of candidates and number of scores"
...
#         for i in range(n):
#             for j in range(m):
#                 value[i] += " + " + str(scoring[j] * OWA[z]) + " " + "y" + str(pos)
#                 pos += 1
#
#     for i in range(n):
#         f.write("v%d: -z %s >= 0\n" % (i + 1, value[i]))
...
#     #    f.write("Subject To\n")
#
#     # constraint for x's  <--- the constraint that we choose exactly k candidates
#     subj_k = "c1:"
#     first = True
#     for c in candidates:
#         if not first:
...
#
#
# def run_ilp_OWA_egal(election, k, OWA, scoring=None):
#     n, m = election.shape
#
#     candidates = np.sort(election[0, :])
#
#     tmp = "tmp2_" + str(random.randint(0, 3283393292303320932)) + ".lp"
#
#     write_cplex_format_OWA_egal(election, k, OWA, tmp, scoring)
#
#     cpx = cplex.Cplex(tmp)
#
#     cpx.set_log_stream(None)
...
#
#
# def position(i, j, n, m):
#     return i * n + j
#
#
# def write_cplex_format_CC_egal(election, k, out, scoring=None):
#     n, m = election.shape
#
#     if scoring is not None:
#         assert m == len(scoring), "mismatch: number of candidates and number of scores"
...
#     for i in range(n):
#         for j in range(m):
#             value[i] += " + " + str(scoring[j]) + " " + "y" + str(position(i, j, n, m))
#             pos += 1
#
#     for i in range(n):
#         f.write("v%d: -z %s >= 0\n" % (i + 1, value[i]))
...
#             subj_k = "c%d:" % constraint
#             constraint += 1
#             subj_k += " y%d - %s <= 0" % (position(i, j, n, m), d[election[i][j]])
#             f.write(subj_k + '\n')
#
#     # k
#     f.write("Binary\n")
...
#
#
# def run_ilp_CC_egal(election, k, scoring=None):
#     n, m = election.shape
#
#     candidates = np.sort(election[0, :])
#
#     tmp = "tmp2_" + str(random.randint(0, 3283393292303320932)) + ".lp"
#
#     write_cplex_format_CC_egal(election, k, tmp, scoring)
#
#     cpx = cplex.Cplex(tmp)
#
#     cpx.set_log_stream(None)
