...
class ElectionCreator:
    ...
    def create_random_election_with_gaussian_costs(n, m, gauss_mean, gauss_std, limit = -1):
        '''
        n voters over c1,...,cm with Gaussian costs.
        '''
        items = []
    ...
    def create_random_approval_election(n, m, p, limit = -1):
        '''
        n approval voters over c1,...,cm all cost 1
        where independent probability of voter approving some item is p.
        '''
        itemsarray = []
        for j in range(1, m + 1):
            itemsarray.append(Item('c' + str(j), 1))
        items = Items()
